...
(async function(){
  ...
  function renderTabla(){
    ...
    tabla.innerHTML = items.map((it, idx) => `
      <tr class="hover:bg-gray-50">
        ...
        <td class="border border-gray-400 px-4 py-3 text-center text-black">${escapeHtml(it.institucion)}</td>
    ...
    tabla.querySelectorAll('.btn-editar').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        const inInstitucion = document.getElementById('institucionq');
        ...
        if (inInstitucion) inInstitucion.value = it.institucion || '';
    ...
    tabla.querySelectorAll('.btn-eliminar').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        const i = Number(btn.getAttribute('data-index'));
        items.splice(i,1);
        localStorage.setItem('gestion_academica', JSON.stringify(items));
        renderTabla();
      });
    });
  }
  ...
  const stored = localStorage.getItem('gestion_academica');
  if (stored) {
    try {
